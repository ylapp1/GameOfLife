...
/**
 * Sets the cursor position to x|y relative to the current window dimensions.
 *
...
{
    // Check X-Coordinate
    if (_x < 0) printf("Error: The x value exceeds the left console border (x = %d).\n", _x);
    ...
    // Check Y-Coordinate
    else if (_y < 0) printf("Error: The y value exceeds the top console border (y = %d).\n", _y);
    ...
    { // Set the cursor
        ...
        int absoluteY = consoleInformationFetcher.getNumberOfBottomDisplayRow() - numberOfConsoleRows + _y;
...
/**
 * Sets the cursor position to x|y relative to all rows in the output buffer.
 *
